...
Preliminary analysis, improve by:

- correct for NAP redefinition in 2005
- 
...
ggplot(aes(x = year)) +
  geom_col(aes(y = n_per_year/1e3, fill = station), position = position_dodge())# +
...
ddl_y_means_av <- ddl_y_means %>%
  ...
  bind_rows(
    . |>
      group_by(year) |>
      summarise(
        annual_mean_mm_corrected = mean(height)
      ) |>
      mutate(
