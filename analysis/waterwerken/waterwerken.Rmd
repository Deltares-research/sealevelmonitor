...
```{r alleWerken, fig.height=10, fig.width=10}
...
ww %>%
  ...
  geom_point(aes(x = end, y = project), shape = "|", size = 3) +
...
order = ww %>% arrange(-end) %>% select(project) %>% unlist %>% unname
...
ww %>%
  ...
  geom_segment(aes(x = start, 
                   xend = end, 
                   y = interaction(regio, project),
                   color = regio),
               linewidth = 2, 
               alpha = 0.6#, 
               # arrow = grid::arrow(ends = "last", type = "open")
