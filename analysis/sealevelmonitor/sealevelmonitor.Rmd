...
```{r variation-model-time, fig.height=nstations*2+1, fig.width=nmodels*2.5+1, fig.cap="De verdeling van residuwaarden uitgezet tegen de tijd in jaren. "}
models %>%
...
Figuur \@ref(fig:variation-model-time) laat zien dat de spreiding van residuen groter is voor de jaren vóór 1950. Dit zijn de jaren waarvoor geen correctie van opzet wordt uigevoerd, omdat hiervoor geen GTSM berekeningen beschikbaar zijn. 
...
```{r}
# internal check
# extra ZSM after 1960

square_term <- parameterTable[parameterTable$short_term == "+ square_trend 1960",]$estimate
Trend <- parameterTable[parameterTable$short_term == "Trend",]$estimate

Totaltrend_from_parameters = Trend + square_term * 2 * (2024 - 1960)

# numerically fromm preidctions table
Totaltrend_from_predictions <- all_predictions %>% 
  mutate(ZSM = prediction_recalc - lag(prediction_recalc, n = 1, order_by = data_year)) %>% 
  select(ZSM) %>% 
  tail(1) %>%
  unlist() %>% 
  unname()
...
print(
  c(Totaltrend_from_parameters,  Totaltrend_from_predictions)
)
