---
title: "Zeespiegelmonitor analysis"
...
```{r zeespiegelmetingen, fig.width=7, fig.height=5, fig.cap="Yearly averaged sea level for the six main tidal stations in the Netherlands."}
...
- Harlingen show a lower trend as compared to the average of the stations from roughly 1930 to 1990.
- IJmuiden shows strong fluctuations in the first decades of the series that are not seen at the other stations. in 2018, the signal drops a few cm. 
...
For the combined stations Netherlands and Netherlands (without Delfzijl), the `r cat(preferred_model)` model has the lowest AIC, and is therefore the first candidate for the preferred model. In the next section, it is tested whether the broken-linear model explains the observed variation significantly better than the simplest model, the linear model.
...
```{r prediction-plot_preferred-model, eval=T, fig.height=5, fig.width=8, fig.cap= "Observed (corrected for surge anomaly) and predicted sea level for all main stations and the preferred model. "}
...
```{r prediction-plot_preferred-model2, eval=T, fig.height=3, fig.width=8, fig.cap= "Observed (corrected for surge anomaly) and predicted sea level for averaged station and the preferred model. "}
...
```{r zss-mainstations, fig.width=6.5, fig.height=3, fig.cap = "Sea level rate before (green) and after (red) 1993, where the breakpoint is located for the preferred model (see above)."}
