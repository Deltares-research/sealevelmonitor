...
Een van de factoren die de zeespiegelhoogte beïnvloeden is windopzet. De jaarlijks gemiddelde windopzet wordt berekend met het Global Tide and Surge Model (\acr{GTSM}). 

TODO: GTSM uitleg en referenties
- GTSM - depth-averaged hydrodynamic model for calculating tides and surges in the global ocean, and their non-linear interactions.
- In ZSM we use the GTSM-ERA5 dataset, providing a reanalysis of global water levels from 1950 to present (hourly values), generated with the ERA5 reanalysis as atmospheric forcing. 
- Surge is determined as a difference between the surge-tide and tide-only simulations, and is provided as a separate variable in the dataset.
- reference to the dataset: https://doi.org/10.24381/cds.a6d42d60
- reference to papers describing it: Muis et al. 2023 https://doi.org/10.1029/2023EF003479

Modelresultaten voor windopzet (surge) uit GTSM runs worden geselecteerd per hoofdstation. Deze worden daarna gemiddeld per maand voor elk hoofdstation en opgeslagen voor later gebruik in de Zeespiegelmonitor (zie dit [rekendocument](https://github.com/VU-IVM/gtsm3-era5-nrt/blob/main/03_other_postprocessing/00_monthly_means_surge_SeaLevelMonitor.ipynb)).
...
```{r GTSM-monthly-means, fig.cap="Maandgemiddelde windopzet voor alle jaren over de periode 1950 - 2025 voor Hoek van Holland. Zie tekst voor meer uitleg. "}
knitr::include_graphics("images/GTSM_monthly_means_surge_HoekVanHolland_1950-2025.png")
```

```{r GTSM-monthly-means, fig.cap="Windopzet tijdreeks en maandgemiddelde windopzet voor 2025 voor Hoek van Holland. Zie tekst voor meer uitleg. "}
knitr::include_graphics("images/GTSM_monthly_means_surge_HoekVanHolland_2025_timeseries.png")
```

```{r GTSM-yearly-means, fig.cap="Jaargemiddelde windopzet voor 1950-2025 voor alle stations. Zie tekst voor meer uitleg. "}
knitr::include_graphics("images/GTSM_yearly_means_surge_all_stations.png")
```
...
```{r GTSM-SLR-diff, fig.cap="Gemiddelde windopzet op de zeespiegel voor 2023 inclusief zeespiegelstijging zoals gedefinieerd in GTSM (linker figuur) en het verschil in windopzet tussen modelruns met en zonder zeespiegelstijging (rechter figuur). Voor verder uitleg zie de tekst."}
